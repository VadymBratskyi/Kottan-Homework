...
namespace Battleship.Tests
{
    public class ShipParseShould
    {
        ...
        public void ParseNotation(string notation, uint expectedX, 
            uint expectedY, uint expectedLength, Direction expectedDirection)
        {
            var position = Boats.Parse(notation);
        ...
        {
            Assert.Throws<NotABoatException>(() =>
            {
                Boats.Parse(notAShip);
            });
        }
        ...
        {
            Assert.IsInstanceOf(expectedType, Boats.Parse(ship));
        }
    }
}